...
class start_make_model(nn.Module):
    ...
    def __init__(self, config_args, args, mean_std):
        ...
        self._in_feat = 3
        self._hidden_dim = model_args['d_model']
        self._node_dim = 10
        self._forecast_dim = 256
        self._output_hidden = 512
        self._output_dim = task_args['pred_num']

        self._num_nodes = data_args[args.source_dataset]['node_num']
        self._k_s = 2#model_args['k_s']
        self._k_t = 3#model_args['k_t']
        self._num_layers = 4#5

        model_args['use_pre'] = False
        model_args['dy_graph'] = True
        model_args['sta_graph'] = True
        model_args['node_hidden'] = 10
        model_args['time_emb_dim'] = 10
        model_args['seq_length'] = task_args['pred_num']
        ...
        self.T_i_D_emb = nn.Parameter(
            torch.empty(288, 10))  #model_args['time_emb_dim']))
        self.D_i_W_emb = nn.Parameter(
            torch.empty(7, 10))#model_args['time_emb_dim']))
        ...
        self.out_fc_2 = nn.Linear(self._output_hidden, 3)#model_args['gap'])
